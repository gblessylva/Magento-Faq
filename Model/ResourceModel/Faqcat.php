...
class Faqcat extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    const FAQ_CATEGORY_FILE_PATH_UPLOADED = 'buyrite'.DIRECTORY_SEPARATOR.'faq'.DIRECTORY_SEPARATOR.'category'.DIRECTORY_SEPARATOR;

    const FAQ_CATEGORY_FILE_PATH_ACCESS = 'buyrite/faq/category/';
    ...
    {
        $this->_init('buyrite_faq_category', 'category_id');
    }
    ...
    protected function _afterLoad(AbstractModel $object)
    {
        $category_store = $this->getConnection()
            ->select()
            ->from($this->getTable('buyrite_faq_category_store'), ['store_id'])
    ...
    protected function saveCategoryRelation(AbstractModel $object)
    {
        ...
        if ($category_id && (int) $category_id > 0) {
            ...
            if ($stores) {
                ...
                $adapter->delete($this->getTable('buyrite_faq_category_store'), $condition);
                ...
                $adapter->insertMultiple($this->getTable('buyrite_faq_category_store'), $data);
    ...
    public function getFaqCategoryStore($category_id = null)
    {
        ...
        $select = $this->getConnection()->select()
            ...
            ->joinLeft(
                ['faqcat_store' => $this->getTable('buyrite_faq_category_store')],
    ...
    public function getCategoriesList()
    {
        ...
        $select = $adapter->select()
            ->from(['cat' => $this->getTable('buyrite_faq_category')])
            ->join(['cat_store' => $this->getTable('buyrite_faq_category_store')], 'cat.category_id = cat_store.category_id', ['store_id'])
